...
public class TestSimpleSearchAndSort extends BaseTestCase {
    ...
    void canSearchAndSortByPriceHighToLow() {
        ...
        float previousPrice = Float.parseFloat(user.asksFor(AtProductListTable.forProductPrice(1)));
        for (int i = 1; i <= productsDisplayed; i++) {
            float currentItemPrice = Float.parseFloat(user.asksFor(AtProductListTable.forProductPrice(i)));
            user.attemptsTo(
